...
import agentic_ecal_pkg as ae  # noqa: E402
...
# Amortization: derived two-rate at one serving batch for both curves, so the comparison is
# method-uniform. (AMORT_ETA removed: a single-rate residue, read nowhere once fig_amortization
# moved to the two-rate path at CASE_BATCH.)
#
# LLaMA-65B was dropped. Qwen2.5-7B cannot replace it: it trained on 18T tokens but publishes no
# GPU-hours and no energy, so e_pretrain = 0 and its curve would collapse onto its own operational
# floor with no crossover. Both models below carry disclosed GPU-hours x TDP from the Llama-3
# model card.
AMORT_MODELS = (
...
def fig_amortization(out_dir):
    ...
    Derived two-rate at one serving batch for all three curves: the 65B and 70B have no measured
    calibration of their own, so a uniform method is the only comparable one. The crossover RATIOS
    between models are invariant to hardware and batch alike, which is why the text quotes ratios,
    not absolutes.
    """
    hw = CASE_HW
...
def write_dimensioning_table(out_dir):
    ...
    tex = f"""% Generated by figure_scripts/make_figures.py -- do not edit by hand.
\\begin{{table*}}[t]
