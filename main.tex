...
\begin{figure*}[t]
  ...
  \caption{\ROOF{}What a distributed team exchanges, and what it costs on the links of
  Table~\ref{tab:placement} other than the NB-IoT uplink. Three topologies over $K=3$ rounds; the
  topologies differ in what a message must carry, not only in how many there are: a chain
  successor needs the whole conversation, a tree child only its parent's instruction, and a mesh
  agent broadcasts its own latest output to every peer. (a) Message count and data volume move in
  opposite directions---at $N=32$ the mesh sends $2976$ messages against the chain's $96$, yet
  moves less data ($12.7$ against $19.8$\,Mbit), because a chain message carries the accumulated
  transcript. (b) The resulting energy spans three orders of magnitude across the four links.
  (c) Against the compute it accompanies, on the most constrained of them, it stays between
  $0.2\%$ and $1.2\%$ for every topology and team size.\OLD{}}
  \label{fig:comm}
\end{figure*}
...
\ROOF{}Fig.~\ref{fig:comm} separates the two quantities that a message count alone
conflates. Across the three topologies the number of messages and the data they carry scale in
opposite directions: a full mesh over $32$ agents sends $2976$ messages against a chain's $96$,
yet moves \emph{less} data, $12.7$ against $19.8$\,Mbit, because a mesh message carries one
agent's latest output while a chain message carries the transcript accumulated so far. The tree is
cheapest on both counts, $0.41$\,Mbit in the same $96$ messages, since a child needs only its
parent's instruction. Priced over the links of Table~\ref{tab:placement}, that data spans three
orders of magnitude in energy, from $0.08$\,J for the tree on metro fibre to $119$\,J for the
chain on a loaded edge cell at $N=16$. Set against the compute it accompanies, however, it remains
between $0.2\%$ and $1.2\%$ on every one of those links and at every team size, and the reason is
structural rather than incidental: a token that crosses the network must also be prefilled by its
receiver, and prefill costs $603\times$ more per token than a 5G bearer moves it ($4075\times$ on
fibre). The topology therefore determines the traffic a distributed deployment must carry---which
is a link-dimensioning question---without determining its energy.\OLD{}
