...
\begin{figure*}[t]
  ...
  \includegraphics[width=\textwidth]{figures/fig_components.pdf}
    \caption{\ROOF{}Where workflow energy goes, and how the agent graph moves it. Calls are priced
  with Eq.~\eqref{eq:tworate} using the measured coefficients of Fig.~\ref{fig:validation}, so the
  prefill and decode bands are its two terms. (a) Composition of a history-carrying chain and of
  fan-out trees of depth one and two, with $E_W$ annotated above each bar. (b) A chain re-reads its
  accumulated transcript, so its prefill share climbs from $45\%$ to $83\%$ as the workflow grows;
  a tree node reads only its parent, so the tree holds near $40\%$ however wide or deep it becomes.
  (c) The consequence per useful bit: the chain costs $6.2\times$ more at $86$ calls than at $4$,
  the tree $1.07\times$.\OLD{}}
